...
class AdminServiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->coustomize_icon != null) {
            $request->validate([
                'coustomize_icon' => ['mimes:jpeg,png,gif'],
            ],[
                'coustomize_icon.mimes' => __('Banner must be jpeg, png, jpg or gif'),
            ]);
            $final_name1 = 'coustomize_icon'.time().'.'.$request->coustomize_icon->extension();
            $request->coustomize_icon->move(public_path('uploads'), $final_name1);
            $service->coustomize_icon = $final_name1;
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->coustomize_icon != null) {
            ...
            if($service->coustomize_icon) {
                unlink(public_path('uploads/'.$service->coustomize_icon));
            }
            $final_name1 = 'coustomize_icon'.time().'.'.$request->coustomize_icon->extension();
            $request->coustomize_icon->move(public_path('uploads'), $final_name1);
            $service->coustomize_icon = $final_name1;
